...
#define READ_CHECKED(fd, buf, byte_len)     if (fread(buf, 1, byte_len, fd) < byte_len)\
                                            {\
...
static inline int parse_data_chunk(t_wav_parser *p)
{
    ...
    unsigned long old_size = p->sample->num_samples * (p->codec->bits_per_sample / 8);
    ...
    p->sample->num_samples += byte_length / (p->codec->bits_per_sample / 8);
    int r = fread(p->sample->samples[0] + old_size, 1, byte_length, p->fd);

    return RET_OK;
}


int wav_dec_parse(FILE *fd, t_sample *sample)
{
    assert(fd != NULL && "Invalid file");
